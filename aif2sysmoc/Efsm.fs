...
//    let checkForInOut =  aifsys.iface |> List.iter (fun (x,y) -> if y.flow = Averest.Core.Declarations.Flow.InOut then failwith "The AIF system has In-Out ports ! Please modify the system to use In and/or Out ports excusively")
    ...
    let _startLabel = 
        ...
        match x with
            ...
            | _ -> false)
        |> (fun (x,y) ->  Utils.LhsQnameofAction(y))
...
//
    ...
    member private this.SetLabelValuesToTrue (inlabels:QName list) =
        ...
        _labelValsMap|> Map.map(fun qname value -> 
                                if labels.Contains(qname) || qname = _startLabel.Value then
    ...
    member this.Initialize () = 
        ...
        let InitializeLabelGuards = 
            ...
            let buildLabelDict (grdaction:Actions.GrdAction) =
                let (boolexp, qname) = (fst(grdaction), Utils.LhsQnameofAction(snd(grdaction)).Value)
...
//                    |> List.iter(fun (transition,valid) ->  GenerateGraphDFS(transition.NextState))
        ...
        let GenerateGraphInit = 
            ...
            let currentState = this.MakeState([_startLabel.Value])
        ...
        let rec PrintGraph (currentStateList:EFSMState list) = 
            ...
            if not (currentStateList.IsEmpty) then
                ...
                currentStateList |> List.iter (fun currentState ->
                                                    ...
                                                    _efsmGraph.GetTransitions(currentState)
                                                                |> List.iter(fun transition ->
                                                                                    if      not (currentStateMap.ContainsKey(transition.NextState.Labels)) // state won't                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           